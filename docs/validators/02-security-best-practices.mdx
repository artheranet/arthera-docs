...
## Setting up a minimum level of security on your server:
...
4. The next step is setting up two or three general policies that will help secure your system by blocking incoming connections and allowing outbound connections:
   ...
   `sudo ufw allow ssh` **or** if disabling it `sudo ufw deny ssh`
...
6) After hardening our system we need to open up port 6534 to ensure that our validator can communicate with the rest of the network:
   `sudo ufw allow 6534`
...
2. Switching to the location of the configuration file:
   `cp /etc/fail2ban/jail.conf /etc/fail2ban/jail.local`

3. Edit the configuration options:
   `nano jail.local`

4. Find the **[DEFAULT]** section and edit these options:
