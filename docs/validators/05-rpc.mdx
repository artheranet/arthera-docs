...
```bash
docker pull arthera/arthera-node:1.2.1-rc.2
docker run --name arthera-rpc --network host -v $HOME/arthera:/data arthera/arthera-node:1.2.1-rc.2 \
  --port 6533 \
...
docker pull arthera/arthera-node:1.2.1-rc.2
docker run -d --restart unless-stopped --name arthera-rpc --network host -v $HOME/arthera:/data arthera/arthera-node:1.2.1-rc.2 \
  --port 6533 \
  --genesis /data/mainnet.g --genesis.allowUnknown \
  --syncmode full \
  --nodekey /data/node.key \
...
d6ac0f5fdf14   arthera/arthera-node:1.2.1-rc.2  "arthera-node --data…"   2 seconds ago   Up 1 second             arthera-rpc
```
