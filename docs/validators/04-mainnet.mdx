...
```bash
docker pull arthera/arthera-node:1.2.0-mainnet-rc.5
```
...
```bash
docker run -it -v $HOME/arthera:/data arthera/arthera-node:1.2.0-mainnet-rc.5 account new
```
...
```bash
docker run -it -v $HOME/arthera:/data arthera/arthera-node:1.2.0-mainnet-rc.5 validator new
```
...
```bash
docker run -it -v $HOME/arthera:/data arthera/arthera-node:1.2.0-mainnet-rc.5 p2p genkey /data/node.key
```

Check to see if the key was generated successfully by running:

```bash
docker run -it -v $HOME/arthera:/data arthera/arthera-node:1.2.0-mainnet-rc.5 p2p enodeurl /data/node.key
```
...
```bash
curl -o $HOME/arthera/mainnet.g https://s3.eu-central-1.amazonaws.com/release.arthera-net/mainnet-100224.g
```
...
```bash
docker pull arthera/arthera-node:1.2.0-mainnet-rc.5
docker run --name arthera-validator --network host -v $HOME/arthera:/data arthera/arthera-node:1.2.0-mainnet-rc.5 \
  --port 6533 \
...
docker pull arthera/arthera-node:1.2.0-mainnet-rc.5
docker run -d --restart unless-stopped --name arthera-validator --network host -v $HOME/arthera:/data arthera/arthera-node:1.2.0-mainnet-rc.5 \
  --port 6533 \
  --genesis /data/mainnet.g \
  --syncmode full \
  --nodekey /data/node.key \
...
d6ac0f5fdf14   arthera/arthera-node:1.2.0-mainnet-rc.5  "arthera-node --data…"   2 seconds ago   Up 1 second             arthera-validator
```
