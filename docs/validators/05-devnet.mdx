...
Run the command below to download the `devnet2.g` genesis file:
```bash
curl -o $HOME/arthera/devnet.g http://release.arthera.net/devnet2.g
```
...
Choose your validator node from the dropdown list.

The validator id displayed in the dropdown list is actually your enode url from step
[10. Generate your node key](#10-generate-your-node-key) file formatted like `<first 4 chars><middle 4 chars><last 4 chars>`
