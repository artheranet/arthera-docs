...
```bash
docker pull arthera/arthera-node:1.2.0-devnet.rc1
```
...
```bash
docker run -it -v $HOME/arthera:/data arthera/arthera-node:1.2.0-devnet.rc1 account new
```
...
```bash
docker run -it -v $HOME/arthera:/data arthera/arthera-node:1.2.0-devnet.rc1  validator new
```
...
```bash
docker pull arthera/arthera-node:1.2.0-devnet.rc1
docker run -d --restart unless-stopped -p 6535:6060 -v $HOME/arthera:/data arthera/arthera-node:1.2.0-devnet.rc1 \
  --nodekey /data/node.key \
  --nat extip:YOUR_PUBLIC_IP \
  --validator.id ID_FROM_STEP_8 \
  --validator.pubkey "VALIDATOR_PUBLIC_KEY_FROM_STEP_7"
...
d6ac0f5fdf14   arthera/arthera-node:1.2.0-devnet.rc1   "arthera-node --data…"   2 seconds ago   Up 1 second   0.0.0.0:18545-18546->18545-18546/tcp, :::18545-18546->18545-18546/tcp, 0.0.0.0:6535->6060/tcp, :::6535->6060/tcp   heuristic_northcutt
```
