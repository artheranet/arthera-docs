...
A Contract subscription must have an EOA owner that can manage the subscription and the contract must comply
with the following interface:

```solidity
interface ISubscriptionOwner {
    // returns an EOA account that owns the contract subscription
