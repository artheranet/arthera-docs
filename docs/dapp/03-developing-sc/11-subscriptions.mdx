...
The contract itself and must inherit from `@arthera/subscription/ISubscriptionOwner`
and `@openzeppelin/contracts/utils/introspection/ERC165.sol` and add the following imports and methods to its code:
...
To allow users to consume from the contract's subscription, the owner of the contract must whitelist the respective user accounts
using the [whitelistAccount()](/dapp/developing-sc/system-contracts/Subscribers#whitelistaccount) method available in the [Subscribers](/dapp/developing-sc/system-contracts/Subscribers) system contract.

To disallow a whitelisted user from using the contract's subscription, the owner of the contract must use the [blacklistAccount()](/dapp/developing-sc/system-contracts/Subscribers#blacklistaccount)
method available in the [Subscribers](/dapp/developing-sc/system-contracts/Subscribers) system contract.

Querying if an account is whitelisted/blacklisted can be done by invoking the [isWhitelisted()](/dapp/developing-sc/system-contracts/Subscribers#iswhitelisted) method available
in the [Subscribers](/dapp/developing-sc/system-contracts/Subscribers) system contract.
...
For example, a DApp might accept credit card payments through a merchant and a subscription management platform like
[Chargebee](https://www.chargebee.com/), [Recurly](https://recurly.com/) or the open-source [KillBill](https://killbill.io) to
handle automatic monthly payments. This drastically reduces frauds and eliminates scams.
